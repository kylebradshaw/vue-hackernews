...
describe('host', () => {
  test('rturns empty string if url is undefined', () => {
    expect(host(undefined)).toBe('') // #B
  })

  test('returns the host from a URL beginning with http://', () => {
    const url = 'http://google.com' // #A
    expect(host(url)).toBe('google.com') // #B
  })
...
describe('timeAgo', () => {
  ...
  const seconds = (second) => second * 1 // #E
  const minutes = (minute) => minute * seconds(60) // #F
  const hours = (hour) => hour * minutes(60)
  const days = (day) => day * hours(24)

  const unixTime = dateNowTime / 1000 // #G

  test('returns singular minute', () => {
    expect(timeAgo(unixTime - minutes(1))).toBe('1 minute') // #H
  })
  ...
  test('returns day rounded to nearest value', () => {
    expect(timeAgo(unixTime - days(2) + 124)).toBe('2 days') // #H
  })
})
